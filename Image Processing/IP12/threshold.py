...
def get_threshold(src, type='rice'):
    ...
    if type == 'rice':
        # 한 줄로 작성하세요
        p = np.array([hist[i]/(h*w) for i in intensity])
    else:
        p = np.array([hist[i]/(h*w) for i in intensity])
    ...
    for k in range(256):
        # 각각 한 줄로 작성하세요
        q1 = np.sum([p[i] for i in range(k+1)])
        q2 = np.sum([p[i] for i in range(k+1, 256)])
        ...
        m1 = np.sum([i * p[i] for i in range(k+1)]) / q1
        m2 = np.sum([i * p[i] for i in range(k+1, 256)]) / q2

        mg = np.sum([i * p[i] for i in range(256)])

        var1 = (np.sum([i*i*p[i] for i in range(k+1)]) / q1) - np.square(m1)
        var2 = (np.sum([i*i*p[i] for i in range(k+1, 256)]) / q2) - np.square(m2)
        ...
        varb = q1 * np.square(m1 - mg) + q2 * np.square(m2 - mg)

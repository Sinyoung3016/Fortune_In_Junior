...
from IP03.my_filtering import my_padding
...
def my_get_Gaussian2D_mask(msize, sigma=1):
    ...
    gaus2D = (1 / (2 * np.pi * sigma ** 2)) * (np.e ** (-(mask / 2 * sigma ** 2)))
...
def my_get_Gaussian1D_mask(msize, sigma=1):
    fsize = msize // 2
    x = np.mgrid[-fsize:fsize + 1, -fsize:fsize + 1]
    mask = np.zeros((1, msize))
    mask[:] = x[1, 0] ** 2

    gaus1D = (1 / (2 * np.pi * sigma ** 2)) * (np.e ** (-(mask / 2 * sigma ** 2)))
